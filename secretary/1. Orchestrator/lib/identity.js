// ============================================================================
//  identity.js  —  the secretary's trigger tags + reply HEADER, in one place.
...
// Accepted trigger tags (lowercase). SECRETARY_TAG_NEW is the SEED — comma-separated, so more
// than one tag can trigger the secretary (e.g. a PT and an EN spelling), default "@mary". The
// owner can CHANGE the list at runtime by asking her ("change your tag to @assist" — the
// assistant_settings skill), and server.js loads the stored list over this seed at boot.
//
// ORDER MATTERS, in one specific way: NEW_TAGS[0] is the PRIMARY tag — server.js falls back
// to it for ctx.tag. So the owner's order is preserved as given, and matchedTagNew() sorts a
...
// Replace the accepted tags at runtime. MUTATES THE ARRAY IN PLACE — `NEW_TAGS` stays a `const`
// export and every reader that already holds the array sees the change. This is not a style
// choice: `scripts/identity-selftest.mjs` destructures on import (`const { NEW_TAGS } = await
...
// The trigger tag this message starts with (for slicing it off the order), or null. Matches
// against NEW_TAGS only. Tags can differ in length (e.g. "@secretaria"=11 vs "@secretary"=10),
// so callers must slice by the RETURNED tag's length, not a fixed constant.
//
...
//      the shorter "@assist", server.js would slice 7 chars, and the router would be handed
//      "ente marque uma reunião" — every Portuguese command silently corrupted. So: longest
//      tag first. Sort a COPY — NEW_TAGS[0] is the PRIMARY tag (server.js falls back to it for
//      ctx.tag) and must keep the owner's order; sorting NEW_TAGS itself would quietly re-elect it.
//
//   2. THE TAG MUST END. Length order alone is not enough, because the trap also springs on a
//      word that is NOT in the list: with "@assist" live, "@assistant do X" still starts with
//      "@assist", so she would answer a tag she does not have and hand the router "ant do X". A
//      tag not in the list must be GONE, not half-working. Hence endsTag().
