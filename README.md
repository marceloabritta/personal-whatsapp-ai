...
- **`calendar_action`** — reads the chat and **creates**, **edits/reschedules**, or **cancels/deletes** a Google Calendar event. On create it extracts participants, date, time and duration, creates the event and fires the invite email to the attendees — and recognises **recurring** events ("every Monday", "every 2 weeks until August", "5 times", "daily"), writing a real repeating event (create-only). It can also give the meeting a **place**: attach a physical address ("na Rua Augusta 123"), or name a place ("o McDonald's da Faria Lima") and it fills in the full address from its own knowledge, asking you to confirm the looked-up address, or auto-generate a **Google Meet** link ("faz por Meet") — physical or virtual, never both. On create (1:1 chats) it also resolves a guest's **missing email from your Google Contacts** by their phone number — filling it silently on one match, asking which on several — and **saves a freshly-supplied email back** to Contacts for next time (needs the Google `contacts` scope). On edit you reply to the invite with a change ("move it to 4pm", "add carlos@example.com", "rename to Kickoff", "switch it to a video call") — it's confirm-first and stays open so you can keep refining before saving; a location-only change is silent unless you ask it to notify the guests.
- **Audio (handled natively — no dispatched skill).** Reply to a voice message and type `@mary transcribe this` (or "summarise this audio"): the system transcribes the audio with AssemblyAI and folds the text into Mary's turn, so she transcribes it back, summarises it, or acts on it as ordinary text. (The model can't ingest audio directly, so the transcription happens system-side. The old `transcribe_audio` skill is dormant.)
...
- **Flights & general questions (handled natively — no dispatched skill).** Ask for a flight, the
  weather, an exchange rate, the news, the contents of a URL already in the thread, or real
  computation, and Mary answers **inline**: on the same turn she runs a live **web search / web fetch
  / (optional) code execution** and replies in prose in the same WhatsApp message — no separate command,
  no "let me check" line. (The dedicated `flight_search` skill is dormant; flights are now just one of
  these native lookups.) Toggle the toolset with the `NATIVE_TOOLS` env var (see `secretary/.env.example`).

Every `@mary` turn is now **one unified model call** that carries the schema-locked decision, the native
toolset, and adaptive reasoning together — Mary reasons, may search the web, then replies, all in one
...
- Native tools: `code_execution` is shipped but **off by default** (`NATIVE_CODE_EXEC`) — enabling it in the turn-call toolset is a fast-follow pending the live end-to-end check.
