...
from manager.models import BACKLOG, BUILD, MAINT, PLAN, FEATURE  # noqa: E402
...
async def main() -> int:
    ...
    check("plan pipeline has columns", len(pl.columns[PLAN]) == 2)
    check("maintenance pipeline has columns", len(pl.columns[MAINT]) == 2)
    check("build pipeline has columns", len(pl.columns[BUILD]) == 2)
    ...
    plan_gate = pl.by_slug(PLAN, "plan")
    check("the plan column is a gate — the one gate in the plan pipeline", plan_gate.gate)
    check("build has NO gate — the plan was already approved upstream", not any(c.gate for c in pl.columns[BUILD]))

    w = board.workers.ensure(pl.by_slug(PLAN, "idea"))
    c = w.contract()
    check("a worker's contract has entry criteria", bool(c["entry"]))
    check("a worker's contract has exit criteria", bool(c["exit"]))
    check("worker agent name is namespaced", w.agent_name == "plan__idea")
    ...
    check(
        ...
        board.cards[cid].dir == os.path.join("cards", "plan", "idea", f"{cid}-add-password-reset-flow"),
    )
    ...
    c = board.cards[cid]
    check("card walked to the plan gate", c.column == plan_gate.id)
    check("card is still in the plan pipeline", c.pipeline == PLAN)
    check("folder moved with the card", c.dir.startswith(os.path.join("cards", "plan", "plan")))
    files = sorted(os.listdir(board.abs_dir(c)))
    # Reaching the Plan gate also renders the Notion-style plan.html (Board._plan_doc_if_ready).
    check("folder accumulated every column's output", files == ["IDEA.md", "PLAN.md", "plan.html"])
    check("artifacts recorded on the card", "PLAN.md" in c.artifacts)
    check("plan doc generated at the gate", "plan.html" in c.artifacts and "plan.html" in files)
    ...
    check("a non-approval does not cross the gate", board.cards[cid].column == plan_gate.id)

    # ---------------------------------------------------------------
    section("gate -> build pipeline")
    # Approving the plan now ships it in one step: it crosses the plan gate into build, then
    # auto-advances build -> ship (no gates) and lands at the last build column.
    await mgr.handle_card_message(cid, "approve")
    c = board.cards[cid]
    check("approval promoted the card into build", c.pipeline == BUILD)
    check("card auto-advanced to the last build column", c.column == pl.last(BUILD).id)
    check("folder followed into the build tree", c.dir.startswith(os.path.join("cards", "build", "ship")))
    files = sorted(os.listdir(board.abs_dir(c)))
    check("plan artifacts travelled with the card", "PLAN.md" in files and "IDEA.md" in files)
    check("build artifacts were added", "BUILD.md" in files)
    ...
    check("the edited pipeline survives a reload", len(board2.pipelines.columns[PLAN]) == 2)
