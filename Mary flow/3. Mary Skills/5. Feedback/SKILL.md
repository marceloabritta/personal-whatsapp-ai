...
> `listen` turn before dispatching; this skill no longer extracts, asks, or opens a session.
> `manifest.conversation:"orchestrator"`, and `run(ctx)` reads `ctx.info` + `ctx.quoted`, files the
> report via `captureFailure`, sends `logged` (or `logFailed` via `sendFailure` on a write failure),
> and **returns** `{ok,reportPath,title}`. WRITE-FIRST is preserved structurally: the orchestrator
> never opens a skill session, so the report is on disk the moment the complaint is dispatched.
