...
> (rails change (b)) and `run(ctx)` does its own `!quoted?.hasAudio → noAudio` check. Each terminal
> branch **returns** a JSON result (`{ok:true,delivered:"inline"|"file",chars}` or
> `{ok:false,reason:"noAudio"|"noKey"|"downloadFailed"|"empty"|"transcribeFailed"}`) so the model
> reads it back. **The `noAudio` branch sends via plain `ctx.send`, NOT `sendFailure`** — replying
> "reply to an audio to transcribe" is user guidance, not a malfunction (manager ruling; the
> server.js:490 rule). The AssemblyAI download/transcribe logic below is unchanged.
